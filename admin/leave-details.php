...
// code to update the read the notification status
$isread=1;
$did=intval($_GET['leaveid']);  
date_default_timezone_set('Asia/Kolkata');
$admremarkdate=date('Y-m-d G:i:s ', strtotime("now"));
$sql="update leave set is_read=:isread where id=:did";
$query = $dbh->prepare($sql);
$query->bindParam(':isread',$isread,PDO::PARAM_STR);
$query->bindParam(':did',$did,PDO::PARAM_STR);
$query->execute();

// code to take action taken on leave
...
date_default_timezone_set('Asia/Kolkata');
$admremarkdate=date('Y-m-d G:i:s ', strtotime("now"));
$sql="update leave set admin_remark=:description,status=:status,remark_date=:admremarkdate where id=:did";
$query = $dbh->prepare($sql);
...
$sql = "SELECT leave.id as lid,employee.fname,employee.lname,employee.empid,employee.id,employee.gender,employee.phone_no,employee.email_id,leave.leave_type,leave.to_date,leave.from_date,leave.description,leave.posting_date,leave.status,leave.admin_remark,leave.remark_date from leave join employee on leave.empid=employee.id where leave.id=:lid";
$query = $dbh -> prepare($sql);
                                        ...
                                        <tr>
                                            <td style="font-size:16px;"> <b>Employee Name :</b></td>
                                              <td><a href="editemployee.php?empid=<?php echo htmlentities($result->id);?>" target="_blank">
                                                <?php echo htmlentities($result->fname." ".$result->lname);?></a></td>
...
<td colspan="5"><?php
if($result->admin_remark==""){
  echo "waiting for Approval";  
}
else{
echo htmlentities($result->admin_remark);
}
?></td>
 </tr>
...
<td colspan="5">
    <?php
if($result->remark_date==""){
  echo "NA";  
}
else{
echo htmlentities($result->remark_date);
}
?>
</td>
 </tr>
    ...
    <div class="modal-content" style="width:90%">
        <h4>Leave take action</h4>
          <select class="browser-default" name="status" required="">
                                             <!-- Admin decides the leave approval and rejection here -->
